...
float CalcOmniShadowFactor(PointLightData pLight, int shadowIndex)
{
	...
	{
		for(float y = -offset; x < offset; x += offset / (samples * 0.5))
		{
			for(float z = -offset; x < offset; z += offset / (samples * 0.5))
			{
				// .r - get first value
